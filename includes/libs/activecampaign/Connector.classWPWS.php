...
class WPWS_AC_Connector {
	...
	public function dbg($var, $continue = 0, $element = "pre", $extra = "") {
	  ...
	  echo "Vartype: " . esc_html(gettype($var)) . "\n";
	  if ( is_array($var) ) echo "Elements: " . count($var) . "\n";
	  elseif ( is_string($var) ) echo "Length: " . esc_html(strlen($var)) . "\n";
	  if ($extra) {
	  	echo esc_html($extra) . "\n";
	  }
	  echo "\n";
	...
	public function curl($url, $params_data = array(), $verb = "", $custom_method = "") {
		...
		if ($this->debug) {
			echo "<textarea style='height: 300px; width: 600px;'>" . esc_html($debug_str1) . "</textarea>";
		}
